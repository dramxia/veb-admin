...
export function PermissionTable({ permissions }: { permissions: Permission[] }) {
  ...
  return (
    <Box bg="white" borderWidth="1px" rounded="lg" overflow="hidden">
      <HStack p={4}>
        <AuthButton code="system:permission:create" isLoading={loading} onClick={() => run(async () => {
      ...
      <Table size="sm">
        <Thead>
          <Tr><Th>权限码</Th><Th>名称</Th><Th>类型</Th><Th>系统</Th><Th>操作</Th></Tr>
        </Thead>
        <Tbody>
          {permissions.map((permission) => (
            <Tr key={permission.id}>
              ...
              <Td><Badge>{permission.type}</Badge></Td>
              <Td>{permission.isSystem ? '是' : '否'}</Td>
              <Td>
                <HStack>
                  <AuthButton code="system:permission:update" size="xs" isDisabled={loading} onClick={() => run(async () => {
                    const name = prompt('权限名称', permission.name);
                    if (name === null) return;
                    await api(`/api/system/permissions/${permission.id}`, { method: 'PATCH', body: JSON.stringify({ name }) });
                  })}>编辑</AuthButton>
                  <AuthButton code="system:permission:delete" size="xs" colorScheme="red" variant="outline" isDisabled={loading} onClick={() => run(async () => {
                    if (!confirm(`确认删除权限 ${permission.code}？`)) return;
                    await api(`/api/system/permissions/${permission.id}`, { method: 'DELETE' });
                  })}>删除</AuthButton>
                </HStack>
              </Td>
            </Tr>
