...
export function MenuTree({ menus, permissions }: { menus: Menu[]; permissions: Permission[] }) {
  ...
  return (
    <Box bg="white" borderWidth="1px" rounded="lg" overflow="hidden">
      <HStack p={4}>
        <AuthButton code="system:menu:create" isLoading={loading} onClick={() => run(async () => {
          ...
          await api('/api/system/menus', {
            method: 'POST',
            body: JSON.stringify({ name, path, type, permissionCode, parentId, component, visible: true, status: 'ENABLED', sort: 0 }),
          });
        })}>新增菜单</AuthButton>
      </HStack>
      <Table size="sm">
        <Thead>
          <Tr><Th>名称</Th><Th>路径</Th><Th>类型</Th><Th>权限码</Th><Th>状态</Th><Th>系统</Th><Th>操作</Th></Tr>
        </Thead>
        <Tbody>
          {sorted.map((menu) => (
            <Tr key={menu.id}>
              ...
              <Td>
                <HStack>
                  <AuthButton code="system:menu:update" size="xs" isDisabled={loading} onClick={() => run(async () => {
                    ...
                    await api(`/api/system/menus/${menu.id}`, {
                      method: 'PATCH',
                      body: JSON.stringify({ name, ...(component !== null ? { component: component || null } : {}) }),
                    });
                  })}>编辑</AuthButton>
                  <AuthButton code="system:menu:delete" size="xs" colorScheme="red" variant="outline" isDisabled={loading} onClick={() => run(async () => {
