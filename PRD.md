...
- 内置仪表盘模块使用 `/dashboard`，后台管理模块移除仪表盘菜单并按首个可访问页面进入；`/admin` 直接跳转到后台管理首个页面。
...
项目尚未上线，数据库由单一初始化迁移 `20260818000000_init` 建表，只描述当前最终模型，不涉及历史数据转换。生产部署先执行 `prisma migrate deploy`，再按需显式执行 seed 写入内置模块、菜单与超级管理员授权。
