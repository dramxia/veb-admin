...
function buildHeaders(init: RequestInit) {
  if (isFormDataBody(init.body as BodyInit | null | undefined)) return init.headers;
  return { 'Content-Type': 'application/json', ...(init.headers || {}) };
}

function parseApiPayload<T>(text: string, status: number): ClientApiResult<T> {
  if (!text.trim()) {
    return { code: status, data: null, message: status >= 400 ? '服务暂时不可用' : 'ok' };
  }
...
export async function requestJson<T>(path: string, init: RequestJsonOptions = {}) {
  ...
  if (!response.ok || payload.code !== 0) {
    throw new ClientApiError(payload.message || '操作失败', response.status, payload.code);
  }
