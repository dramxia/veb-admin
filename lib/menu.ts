import { CommonStatus, type Menu, type PermissionType } from '@prisma/client';
...
function buildTree(items: Menu[], options?: { onlyVisible?: boolean }) {
  const filtered = options?.onlyVisible ? items.filter((item) => item.visible) : items;
...
export async function getUserMenuAndPermissions(userId: string): Promise<UserMenuAndPermissions> {
  ...
  const accessibleMenus = menus.filter((menu) => !menu.permissionCode || allowed.has(menu.permissionCode));
...
export async function getMenuByPath(pathname: string) {
  ...
  const menus = await prisma.menu.findMany({ where: { status: CommonStatus.ENABLED } });
  return menus
    .filter((menu) => normalized === menu.path || normalized.startsWith(`${menu.path}/`))
    .sort((a, b) => b.path.length - a.path.length)[0] ?? null;
}

export function permissionCodesByType(
  codes: string[],
