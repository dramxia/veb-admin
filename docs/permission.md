...
角色分配和角色访问范围配置除了要求对应的操作权限，还必须满足服务端委派边界：

- 只有当前有效角色包含 `superadmin` 的操作者才能增加或移除 `superadmin` 关系。
- 非 `superadmin` 只能分配其自身有效模块和权限码能够完整覆盖的角色；检查同时包含目标
  用户已有角色，防止借全量替换接口移除更高权限角色。
- 非 `superadmin` 只能修改其自身有效授权能够完整覆盖的普通角色，且新访问范围中的模块和
  菜单权限码也不得超出操作者当前有效授权。
- 校验在 VEB API 执行，前端角色选项和按钮隐藏不作为安全边界。
...
- 非 `superadmin` 操作者只能选择自身当前有效授权覆盖的模块和权限码。
...
- `/admin/system/role`：通过“配置访问权限”全量维护角色的模块和节点，通过“分配用户”
  全量维护该角色的用户；两类写操作都受服务端委派边界限制。
- 用户管理中的“分配角色”全量维护指定用户的 `UserRole`，新增用户携带角色时使用相同
  委派边界。
