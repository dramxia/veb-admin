...
- Header、Dock、Modal、Drawer、Menu、Popover 可使用玻璃态。
- 表格外层、数据卡片可使用弱玻璃态。
...
后台主结构由 Header、侧边主导航和主内容区组成；窄屏可将导航收敛为固定底部 Dock。
...
- 主内容区最大宽度建议为 `1280px`。
- 侧边导航在桌面端保持稳定宽度；底部 Dock 仅作为窄屏导航变体，固定时需预留 `safe-area-inset-bottom`。
- 采用底部 Dock 时，主内容底部必须预留空间，避免遮挡分页、按钮和表格底部内容。
...
当前项目仅在侧边导航使用 framer-motion；新增动效必须延续本节规则。
