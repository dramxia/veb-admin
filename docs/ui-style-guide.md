...
- Header、半圆导航、Modal、Drawer、Menu、Popover 可使用玻璃态。
- 表格外层、数据卡片可使用弱玻璃态。
...
业务组件不得新增硬编码的 `rgba(...)`、`backdrop-filter`、`box-shadow` 或渐变字符串。当前存量代码中的此类值应在相关组件改动时逐步收敛到共享 token 或变体，不能继续复制扩散。右下 CSS 伪 3D 导航是唯一例外：其渐变材质、透视、伪元素高光和投影集中维护在 scoped CSS Module 中，不得复制到其他业务组件。
...
后台主结构由居中的 Header、居中的主内容区和右下半圆主导航组成；导航不占用文档流宽度。
...
- 主内容区最大宽度建议为 `1280px`。
- 半圆导航在桌面端始终展开，弧面向左、右侧直径贴近视口边缘。
- 窄屏默认只露出导航把手，展开后必须保留 `safe-area-inset-bottom`，装饰层不得拦截下方页面点击。
...
- 半圆导航在窄屏下缩小尺寸并默认收起，不缩小文字或球体到不可操作尺寸。
...
当前项目在右下半圆导航和页面微交互中使用 framer-motion；新增动效必须延续本节规则。
...
目标：以固定的 CSS 伪 3D 球体形成轻盈、现代且可访问的主导航。

规则：

- 固定在视口右下角，桌面端展开，窄屏通过把手展开。
- 使用向左展开的半圆、渐变透明直径和伪元素厚度表现空间感，不使用 WebGL 或 Canvas。
- 最终可导航页面按排序映射为球体，每屏最多 8 个，超出后在直径侧分页。
- 当前球体保留名称标签，其他球体通过 Tooltip 展示名称。
- 内部路由在球体沿贝塞尔曲线到达主内容中心后切换，并尊重 reduced motion。
- 装饰层使用 `pointer-events: none`，仅交互按钮接收事件，避免阻断被覆盖内容。
