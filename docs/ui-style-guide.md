...
- `OverlayStackProvider`、`AppModal`、`AppDrawer`、`AppAlertDialog`：统一管理全局浮层栈和 Portal Root。所有已打开浮层保持挂载和可见，共用一个 `RemoveScroll` 页面滚动锁；只有最上层接管焦点锁、背景惰性、Escape 和遮罩关闭，关闭后外层浮层继续使用原状态。ESLint 禁止业务代码直接使用 Chakra 的根级 `Modal`、`Drawer`、`AlertDialog`。
...
- 页面标题和操作区在窄屏改为纵向排列。
- 文章编辑页固定在工作区剩余高度内；返回与保存操作始终位于顶部。桌面端文章信息栏独立滚动，发布状态使用开关控制，开启并保存后即正式发布；Markdown 默认使用分栏模式，编辑与预览按滚动进度双向同步；窄屏在正文和文章信息之间切换。
- 文章列表可直接切换正式发布与草稿状态，并在当前页面的滚动弹窗中按需加载文章预览；预览通过管理接口读取，因此未发布草稿也可查看且不会进入公开文章页。标签关联文章抽屉上叠加文章预览时，抽屉保持挂载、可见和原有状态，预览关闭后可直接继续操作抽屉。
