...
vi.mock('../api', () => ({
  ...
  defineApiRoute: (
    ...
    Object.defineProperty(handler, 'routeAccess', { value: access });
    return handler;
  },
...
describe('role assignment detail route permissions', () => {
  ...
  it('guards access options with assign-access instead of module or menu view', async () => {
    ...
    expect(
      (getAccess as unknown as { routeAccess: unknown }).routeAccess,
    ).toEqual({
      access: 'private',
      permission: 'system:role:assign-access',
    });
  ...
  it('guards user options with assign-user instead of user view', async () => {
    ...
    expect(
      (getUsers as unknown as { routeAccess: unknown }).routeAccess,
    ).toEqual({
      access: 'private',
