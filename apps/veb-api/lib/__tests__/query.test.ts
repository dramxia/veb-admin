...
describe('VEB query parsing', () => {
  it('parses contract-backed pagination and computes database offsets', () => {
    const query = readQuery(
      new Request(
        'http://veb-api.test/api/v1/system/users?page=3&pageSize=25&status=ENABLED',
      ),
      userListQuerySchema,
    );
  ...
  it('rejects malformed pagination and enum values', () => {
    expect(() =>
      readQuery(
        new Request('http://veb-api.test/api/v1/system/users?page=NaN'),
        userListQuerySchema,
      ),
    ).toThrow();
    expect(() =>
      readQuery(
        new Request('http://veb-api.test/api/v1/system/users?status=UNKNOWN'),
        userListQuerySchema,
      ),
    ).toThrow();
  });
