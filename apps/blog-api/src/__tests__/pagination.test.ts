...
describe('pagination', () => {
  it('uses contract defaults and parses valid query strings', () => {
    expect(
      readQuery(
        new Request('https://blog.test/api/v1/public/articles'),
    ...
    expect(
      readQuery(
        new Request(
          'https://blog.test/api/v1/public/articles?page=3&pageSize=50&tag=typescript',
        ),
        publicArticleListQuerySchema,
      ),
    ).toEqual({
  ...
  it('rejects malformed values at the contract boundary', () => {
    expect(() =>
      readQuery(
        new Request(
          'https://blog.test/api/v1/public/articles?page=nope&pageSize=-1',
        ),
        publicArticleListQuerySchema,
      ),
    ).toThrow();
