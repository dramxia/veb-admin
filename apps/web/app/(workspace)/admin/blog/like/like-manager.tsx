...
import type { PageResult } from '@/components/blog/admin-types';
...
export function LikeManager({
  ...
  initial: PageResult<ArticleLike>;
  ...
  const [deleting, setDeleting] = useState<ArticleLike | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [nextData, nextStats] = await Promise.all([
        requestJson<PageResult<ArticleLike>>(
          `/api/v1/blog/manage/likes?${queryString}`,
        ),
        requestJson<LikeStats>(`/api/v1/blog/manage/likes/stats?${statsQuery}`),
      ]);
      setData(nextData);
      setStats(nextStats);
      setSelected([]);
  ...
  function askDelete(item: ArticleLike) {
    setDeleting(item);
    setBatchMode(false);
    confirm.onOpen();
  }
  ...
  async function confirmDelete() {
    const ok = await feedback.run(
      async () => {
        if (batchMode)
          await requestJson('/api/v1/blog/manage/likes/batch-delete', {
            method: 'POST',
            body: JSON.stringify({ ids: selected }),
          });
        else if (deleting)
          await requestJson(`/api/v1/blog/manage/likes/${deleting.id}`, {
            method: 'DELETE',
          });
      },
