...
export function LikeManager({
    ...
    try {
      ...
      const [nextData, nextStats] = await Promise.all([
        requestJson<PageResult<ArticleLike>>(
          `/api/v1/blog/likes?${queryString}`,
        ),
        requestJson<LikeStats>(`/api/v1/blog/likes/stats?${statsQuery}`),
      ]);
      setData(nextData);
      setStats(nextStats);
      setSelected([]);
  ...
  async function confirmDelete() {
    const ok = await feedback.run(
      async () => {
        if (batchMode)
          await requestJson('/api/v1/blog/likes/batch-delete', {
            method: 'POST',
            body: JSON.stringify({ ids: selected }),
          });
        else if (deleting)
          await requestJson(`/api/v1/blog/likes/${deleting.id}`, {
            method: 'DELETE',
          });
      },
