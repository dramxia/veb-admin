...
export function TagManager({ initial }: { initial: PageResult<AdminTag> }) {
  ...
  const loadList = useCallback(async () => {
    ...
    try {
      ...
      setData(
        await requestJson<PageResult<AdminTag>>(`/api/v1/blog/tags?${query}`),
      );
    } catch (error) {
  ...
  async function submit(event: FormEvent) {
    ...
    const ok = await feedback.run(
      async () => {
        await requestJson(
          editing ? `/api/v1/blog/tags/${editing.id}` : '/api/v1/blog/tags',
          {
    ...
    try {
      setRelated(
        await requestJson<PageResult<ArticleListItem>>(
          `/api/v1/blog/tags/${tag.id}/articles?page=1&pageSize=100`,
        ),
      );
    } catch {
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          const ok = await feedback.run(
            async () => {
              if (deleting)
                await requestJson(`/api/v1/blog/tags/${deleting.id}`, {
                  method: 'DELETE',
                });
            },
            { successTitle: '标签已删除' },
